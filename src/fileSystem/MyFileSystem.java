...
public class MyFileSystem implements FileSystem {
    ...
    public int read(int fd, byte[] buffer) throws IOException {
        ...
        for(off = 0; off < limit; off += len) {
            block = getDirectBlock(fd, MODE.r);//SEEHERE
    ...
    public int write(int fd, byte[] buffer) throws IOException {
        ...
        for(off = 0; off < buffer.length; off += len) {
            if((block = getDirectBlock(fd, MODE.w)) == null) {//SEEHERE
    ...
    private DirectBlock getDirectBlock(int fd, MODE mode) {//FIXME!!!
    	...
    	// return a DirectBlock object representing the direct block
    	// given the current seek position in the open file identified by fd.
    	Inode inode   = fileTable.getInode(fd);
        ...
        if (blockNum > 9) {
        	int singlyCount = IndirectBlock.COUNT;
            int doublyCount = IndirectBlock.COUNT * IndirectBlock.COUNT;
            int triplyCount = IndirectBlock.COUNT * IndirectBlock.COUNT * IndirectBlock.COUNT;   
            IndirectBlock ib1, ib2, ib3;

        	// case 1: singly IndirectBlock needed
            if (blockNum < 10 + singlyCount) {
            	// check if inode ptr is 0, and mode is r
            	// construct singly IndirectBlock
                ib1 = constructIB(inode, 10, mode);
                if (ib1 == null) {
                	return null;
                }
            	return getDirectBlockByIB(ib1, blockNum - 10, inode.ptr[10], blockOff, mode);
            }
            // case 2: doubly IndirectBlock needed
            if (blockNum < 10 + singlyCount + doublyCount) {
            	int i0 = blockNum - 10 - singlyCount;
            	int i1 = i0 / singlyCount;
            	int i2 = i0 % singlyCount;
            	// get singly IndirectBlock
            	ib1 = constructIB(inode, 11, mode);
                if (ib1 == null) {
                	return null;
                }
            	// get doubly IndirectBlock
                ib2 = constructIB(ib1, i1, inode.ptr[11]);
                if (ib2 == null) {
                	return null;
                }
            	return getDirectBlockByIB(ib2, i2, ib1.ptr[i1], blockOff, mode);
            }
            // case 3: triply IndirectBlock needed
            if (blockNum < 10 + singlyCount + doublyCount + triplyCount) {
            	int i0 = blockNum - 10 - singlyCount - doublyCount;
            	int i1 = i0 / doublyCount;
               	int i2 = (i0 - i1 * doublyCount) / singlyCount;
               	int i3 = (i0 - i1 * doublyCount) % singlyCount;
               	// get singly IndirectBlock
               	ib1 = constructIB(inode, 12, mode);
                if (ib1 == null) {
                	return null;
                }
            	// get doubly IndirectBlock
                ib2 = constructIB(ib1, i1, inode.ptr[12]);
                if (ib2 == null) {
                	return null;
                }
            	// get triply IndirectBlock
                ib3 = constructIB(ib2, i2, ib1.ptr[i1]);
                if (ib3 == null) {
                	return null;
                }
            	return getDirectBlockByIB(ib3, i3, ib2.ptr[i2], blockOff, mode);
            } else {
    ...
    private IndirectBlock constructIB(Inode inode, int index, MODE mode) {
    	IndirectBlock ib = new IndirectBlock();
    	boolean fresh = inode.ptr[index] == 0;
    	if (!fresh) {
    		// use disk.read() to point to IndirectBlock.
			disk.read(inode.ptr[index], ib);
		} else {
			inode.ptr[index] = freeMap.find();
			if (inode.ptr[index] == 0) { // disk full
    			return null;
    		} else { // find a free data block
    			// write indirect block into disk by inode pointer
    			disk.write(inode.ptr[index], ib);
    			freeMap.save();
    		}
    	}
    	return ib;
    ...
    private IndirectBlock constructIB(IndirectBlock ib1, int i1, int blockNum) {
    	IndirectBlock ib2 = new IndirectBlock();
    	boolean fresh = ib1.ptr[i1] == 0;
    	if (!fresh) {
    		disk.read(ib1.ptr[i1], ib2);
		} else {
			ib1.ptr[i1] = freeMap.find();
			if (ib1.ptr[i1] == 0) {
    			return null;
    		}
			disk.write(blockNum, ib1);
			freeMap.save();
    	}
    	return ib2;
    }

    private DirectBlock getDirectBlockByIB(IndirectBlock ib, int pt, int blocknum, int blockOff, MODE mode){
    	/*
    	 * 	In read mode:
    	...
    	// return DirectBlock
    	// Disk disk, int blockNum, int blockOff, boolean fresh
        return new DirectBlock(disk, ib.ptr[pt], blockOff, fresh);
    }
